...
HOMOGLYPH_DICT = {
    ...
    'Α': ['A', 'А'],  # Greek Alpha (reverse)
    'А': ['A', 'Α'],  # Cyrillic A (reverse)
...
# AI GENERATED WITHOUT EMBEDDINGS
# plaintext = "Good evening everyone, I am Mayank Somvanshi. From BTech 2nd YEar."

# AI GEENRATED WITH EMBEDDINGS
# plaintext = "Gοоd еνеnіng еνеrуоnе‚ І аm Μауаnk Ѕοmνаnѕhι․ Frοm ΒΤеϲh 2nd ҮΕаr․"

# AI GENERATED WITH EMBEDDDINGS AND MODIFIED
plaintext = "hii, Gοоd еνеnіng еνеrуоnе‚ І am Mayank Ѕοmνаnѕhι a student Frοm ΒΤеϲh 2nd Үeаr․"
...
def add_text(text_id, text, silent=False):
    ...
    for rec in records:
        ...
        if sim > max_similarity:
            max_similarity = sim
            closest_match = {"id": rec["id"], "similarity": sim, "distance": dist, "line": p}
    
    store_simhash_csv(CSV_FILE, text_id, new_hash, format(new_hash, '016x'), len(text), text_hmac)
    
    if not silent:
        if closest_match:
            print(f"{text_id}: sim={closest_match['similarity']:.3f} to '{closest_match['id']}' (dist={closest_match['distance']}) and Line Number {closest_match['line']}")
        else:
            print(f"{text_id}: first entry")
    
    return {"id": text_id, "hash": new_hash, "hmac": text_hmac, "closest": closest_match, "line": p}
...
def verify(presaved_length):
    v1 = abs(LengthOfArray-presaved_length)
